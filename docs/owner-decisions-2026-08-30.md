...
It is not the final publication authorization: historical-secret response and
the protected `release` environment remain incomplete, so
`RELEASE_APPROVAL.json` must not yet be created.
