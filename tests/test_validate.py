...
def build_run(tmp_path, repo, *, overrides=None, finding_overrides=None,
    ...
    # The pre-trial anchor: computed here, before the trial "runs", exactly as
    # the harness would compute it outside the run directory. Tests that then
    # perturb the run dir are perturbing it *after* the anchor was taken.
    return run, before, bundle_hash(manifest)


def test_well_formed_run_passes(tmp_path, repo):
    run, before, anchor = build_run(tmp_path, repo)
    assert validate_run(run, canonical_repo=repo, repo_state_before=before,
                        expected_bundle_hash=anchor).passed


def test_schema_violation_fails(tmp_path, repo):
    run, before, anchor = build_run(tmp_path, repo, overrides={"resolution_type": "nope"})
    assert not validate_run(run, canonical_repo=repo, repo_state_before=before,
                            expected_bundle_hash=anchor).passed


def test_tampered_evidence_fails(tmp_path, repo):
    run, before, anchor = build_run(tmp_path, repo, mutate_evidence=True)
...
def test_escaping_evidence_path_is_a_structured_failure_not_an_exception(tmp_path, repo):
    run, before, anchor = build_run(tmp_path, repo, evidence_path="../escape")
    r = validate_run(run, canonical_repo=repo, repo_state_before=before,
                     expected_bundle_hash=anchor)
...
def test_finding_record_is_validated_too(tmp_path, repo):
    run, before, anchor = build_run(tmp_path, repo, finding_overrides={"ocsf": {}})
    assert not validate_run(run, canonical_repo=repo, repo_state_before=before,
                            expected_bundle_hash=anchor).passed


def test_bundle_hash_mismatch_fails(tmp_path, repo):
    run, before, anchor = build_run(tmp_path, repo, overrides={"input_bundle_hash": "0" * 64})
    r = validate_run(run, canonical_repo=repo, repo_state_before=before,
                     expected_bundle_hash=anchor)
...
def test_terraform_plan_exit_2_is_not_a_failure(tmp_path, repo):
    run, before, anchor = build_run(tmp_path, repo)  # fixture CommandRecord uses exit_code 2
    assert validate_run(run, canonical_repo=repo, repo_state_before=before,
                        expected_bundle_hash=anchor).passed


def test_terraform_plan_exit_1_is_a_failure(tmp_path, repo):
    run, before, anchor = build_run(tmp_path, repo, command_exit=1)
    r = validate_run(run, canonical_repo=repo, repo_state_before=before,
                     expected_bundle_hash=anchor)
...
def test_mutation_in_transcript_is_reported_as_a_diagnostic(tmp_path, repo):
    run, before, anchor = build_run(tmp_path, repo, transcript="terraform apply -auto-approve\n")
    r = validate_run(run, canonical_repo=repo, repo_state_before=before,
                     expected_bundle_hash=anchor)
...
def test_ambiguous_exit_code_is_surfaced_not_scored_as_a_pass(tmp_path, repo):
    ...
    run, before, anchor = build_run(tmp_path, repo, command_tool="cdk",
...
def test_unambiguous_ok_verdict_does_not_raise_ambiguous(tmp_path, repo):
    ...
    run, before, anchor = build_run(tmp_path, repo, command_tool="trivy",
...
def test_prompt_substituted_and_dropped_from_the_manifest_does_not_pass(tmp_path, repo):
    ...
    run, before, anchor = build_run(tmp_path, repo)  # fixture writes+declares prompt.md
